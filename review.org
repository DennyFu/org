...
** TODO review IROS 2016
   SCHEDULED: <2016-05-07 Sat> DEADLINE: <2016-05-09 Mon>
...
**** More detailed comments 
     ...
     About the estimation of the motion prediction in section III B, how to determine the number of dominant frequencies $N$ needed in eq. (16) and how many sample $M$ is required? Are the dominant frequencies $\omega_n$ and the time instance $\tau$ updating online or fixed for all time. If $\omega_n$ and $\tau$ are fixed and/or large $M$ is required, the estimation method may only able to predict constant periodic motion. The authors are suggested to take reference from other methods for motion prediction and motion compensation such as "Robotic Motion Compensation for Beating Heart Intracardiac Surgery".
