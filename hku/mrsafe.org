...
{ziyang
The robot platform for cardiac electrophysiological interventions contains chiefly three parts: MR-safe/conditional catheter robot, MR-based tracking system and 3D surgical roadmap registered with intra-operative MR images. 
The presented catheter robot is MR-safe/conditional, consisting of master part in control room and slave part in MRI room. By adopting rolling diaphragm and hydraulic transmission, slave part can be made of non-metallic materials and driven through the control of master part, offering a MR-safe actuation. 
Robot navigation of the catheter uses MR-based tracking units. The tracking unit consists of micro-coil or LC coil circuit which could operate as MR-based active or semi-active signal markers and enable the real-time, frequent and continuous sampling of instrument positional tracking with respect to (w.r.t.) the image coordinates.
Precise and real-time visual feedback to the operator is provided during the catheter/instrument navigation under the MRI. Sharing the same MR-tracking coordinates same with the MR images can guarantee correct alignment of the virtual catheter/instrument configuration relative to the cardiac imaging models, thereby being of benefit to all kinds MRI-guided interventions.
It is the first catheter robot to be integrated with the three components, attempting to achieve a more secure and effective intervention process under MRI.
}

*** Fast non-rigid image registration
To achieve enhanced visual guidance by re-aligning EP roadmap (Fig. 1) onto the intra-op image, Diffeomorphic Demons (DD) is implemented in our robotic platform. By employing diffeomorphic transformation in the deformation model, DD can ensure invertibility, continuity and legitimacy of the deformation field [10.1016/j.neuroimage.2008.10.040]. However, the computation of DD involves a substantial number of convolution processes, which require repeated access of non-coalesced pixel values. Such access will compel a large amount of memory being continuously fetched to the device cache in runtime. 
...
Fig. 5 Computation performance of Demons registration algorithm of 3D MRI images evaluated on GPUs and FPGA using our proposed framework. 


*** The robot working in MRI
