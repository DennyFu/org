...
MRI-compatibile robotic platforms allows surgeons tele-manipulate interventional devices that are located in the MRI-scanner room.
Robots have to fulfill requirements for i) MR-safety; ii) preservation of image quality; and iii) actuation that are unaffected by the strong MR fields [2]. Therefore, ferromagnetic materials that induce image artifacts should be avoided; Conductive materials may induce eddy currents that disrupt magnetic field homogeneity. 
Common actuation mechanisms for robots working near the isocenter of the MRI system can be classified as 1) electrically driven; 2) pneumatically driven and 3) hydraulically driven.
...
Ultrasonic motors and piezoelectric motors are the most commonly used electrically driven MRI-compatible actuation.
...
Pneumatic motors are capable of operating in MR environment, even inside the scanner, without adversely affecting MR image quality.
MRI-compatible pneumatically driven robots are often designed as master-slave system: The master unit can be electric components such as pumps and cylinder and isolate from the MRI system, guaranteeing negligible MR image artifacts; The slave unit, i.e. the robot, works near MRI where the actuation is transmitted pnuematically via long pipelines through the wave guide from the master unit, thus allowing compact design.
...
Hydraulic transmission is an alternative for designing MRI-compatible master-slave type robot design.
Rather than using pressurized air in the pneumatic transmission mentioned previously,  using nearly incompressible pressurized as media in the long pipeline can reduce actuation delay to offer steady transmission and quick response.
Recently, a novel hydraulic motor has been invented [14] and is promising to be implemented in MRI-guided robots; To enhance transmission efficiency, rolling diaphragms were adopted for avoiding sliding friction that occurs between piston shaft and traditional O-rign tight-sealing (Fig. X). 
hydrauclic motor with rolling diaphragms
Still, fluid leakage is a potential problem. Filling the pressurized fluid also increase difficulty in system set up.
